...
class FilterViewFilter extends acymailingView
{
	...
	function form(){
		...
		$js = "function updateFilter(filterNum){
				...
				(ACYMAILING_J30 ? 'jQuery("#filterarea_"+filterNum+" select").chosen();' : "")."
			}
			...
			function updateAction(actionNum){
				...
				(ACYMAILING_J30 ? 'jQuery("#actionarea_"+actionNum+" select").chosen();' : "")."
			}";
		...
		$js .= "var numFilters = 0;
				...
				function addAction(){
					...
					(ACYMAILING_J30 ? 'jQuery("#allactions select").chosen();' : "")." }";
