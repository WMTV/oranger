		...
		?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center" nowrap="nowrap">
						<?php echo JHTML::_('acyselect.booleanlist', "data[listmail][".$row->listid."]" , '',(bool) ($row->mailid OR (empty($row->mailid) AND empty($this->mail->mailid) AND $filter_list == $row->listid) OR (empty($this->mail->mailid) AND count($this->lists) == 1) OR (in_array($row->listid,$selectedLists))),JText::_('JOOMEXT_YES'),JText::_('JOOMEXT_NO'),'listmail'.$row->listid); ?>
					</td>
				</tr>
		<?php
					$k = 1-$k;
						...
						<script language="javascript" type="text/javascript">
							function updateStatus(selection){
								...
								for(var i=0; i < selectedLists['all'].length; i++)
								{
									<?php
									if(ACYMAILING_J30){
										echo 'jQuery("label[for=listmail"+selectedLists["all"][i]+"0]").click();';
									}
									?>
									window.document.getElementById('listmail'+selectedLists['all'][i]+'0').checked = true;
								}
								...
								for(var i=0; i < selectedLists[selection].length; i++)
								{
									<?php
									if(ACYMAILING_J30){
										echo 'jQuery("label[for=listmail"+selectedLists[selection][i]+"1]").click();';
									}
									?>
									window.document.getElementById('listmail'+selectedLists[selection][i]+'1').checked = true;
								}
							}
						</script>
						<?php
