...
;(function(jQuery){
	...
	jQuery.UnoSlider = function(options, base){
		var jQuerythis = base;
		...
		var defaults_cfg = jQuery.extend(true, defaults, jQuery.UnoSlider.defaults);
		var preset_base_cfg = jQuery.extend({}, defaults, jQuery.UnoSlider.defaults);
		var cfg = jQuery.extend(true, defaults_cfg, options);

		var slides = jQuerythis.find(cfg.slidesTag);
		...
		var lists = jQuerythis.find(cfg.slidesTag).css({
			opacity: 0,
			display: "none"
		});
		...
		if(typeof cfg.width !== 'undefined' && cfg.width !== null && cfg.width){
			jQuerythis.css({width: cfg.width});
		}

		if(typeof cfg.height !== 'undefined' && cfg.height !== null && cfg.height){
			jQuerythis.css({height: cfg.height});
		}
		...
		jQuerythis.main = {
			...
			init: function(){
				...
				// show preloader
				jQuerythis.preloader.init(jQuerythis.main.start);
			},
			...
			start: function(){
				...
				if(slides.length > 1){
					...
					if(cfg.indicator) jQuerythis.navigation.indicator();
					jQuerythis.navigation.generate();
					if(cfg.slideshow.timer) jQuerythis.timebar.set();
					if(cfg.slideshow.hoverPause) jQuerythis.slideshow.pauseOnHover();
				}

				if(cfg.touch && jQuerythis.main.is_mobile()) jQuerythis.touch.init();

				jQuerythis.layer.generate();

				if(cfg.responsive) jQuerythis.main.responsive();
				if(cfg.responsiveLayers) jQuerythis.layer.responsive();

				jQuerythis.main.scaleSlides();

				jQuerythis.tooltip.generate();

				var firstSlide = jQuerythis.children('li:first');

				// show captions and layers on first slide
				setTimeout(function(){
					layerDelay = jQuerythis.layer.show(firstSlide);

					if(cfg.tooltip) jQuerythis.tooltip.show(firstSlide);

					if(slides.length > 1){
						if(cfg.slideshow.autostart){
							layerTimer = setTimeout(function(){
								 jQuerythis.slideshow.start();
							}, layerDelay);
						}
					}
			...
			scaleSlides: function(){
				...
				var div = jQuery('<div class="unoslider_slider_area"></div>').css({overflow: 'hidden', width: '100%', height: jQuerythis.height(), position: 'absolute'});
				...
				slides.find('.unoslider_slider_area div img').each(function(){
					...
					if(typeof jQuery(this).attr('height') === 'undefined')
						jQuery(this).height(theImage.height);

					if(typeof jQuery(this).attr('width') === 'undefined')
						jQuery(this).width(theImage.width);
				});
			...
			generateCurtain: function(content){
				...
				// count dimensions
				block = {
					width: Math.ceil(jQuerythis.width() / vertical),
					height: Math.ceil(jQuerythis.height() / horizontal)
				};

				var blocks_area = jQuery('<div class="unoslider_blocks_area"></div>').css({overflow: 'hidden', width: jQuerythis.width(), height: jQuerythis.height(), position: 'absolute', top: '0px', left: '0px', 'z-index': 3}).appendTo(jQuerythis[0]);
				var blocks = jQuery('<div class="unoslider_blocks"></div>').css({width: jQuerythis.width(), height: jQuerythis.height(), position: 'relative'}).appendTo(blocks_area);

				// generate
				while (generateBlocks) {
					content_cube = jQuery('<div class="unoslider_cube_content"></div>').css({
						position: 'absolute',
						left: -X,
						top: -Y,
					...
					jQuery('<div id="block_' + i + '" class="unoslider_cube"></div>').appendTo(blocks).css({
						'z-index': 2,
						position: 'absolute',
						display: 'none',
						top: Y,
					...
					if (X >= jQuerythis.width()) {
						X = 0;
						Y += block.height;
					}

					if (Y >= jQuerythis.height()) generateBlocks = false;
				}

				// add data (image, custom html...)
				var jQuerycontent  = jQuery(content);
				jQuerycontent.find('video').remove();
				jQuerythis.find('.unoslider_cube_content').html(jQuerycontent);

				// disable clicks when the blocks move
				jQuery('.unoslider_cube').click(function(){
					return false;
				});
			},
			...
			prepare: function(slideData){
				running = true;
				jQuerythis.main.generateCurtain(slideData.html());

				return jQuerythis.main.setDirection();
			},

			// execute animation
			execute: function(direction, duration, currentSlide, nextSlide, handChange, delayedChange){
				if(cfg.tooltip) jQuerythis.tooltip.hide();

				if(delayedChange === false){
					setTimeout(function(){
						jQuerythis.slideshow.switchSlides(currentSlide, nextSlide);
					}, 10);
				}
				...
				currentSlide.delay(delay).animate({
					...
					if(delayedChange === true) jQuerythis.slideshow.switchSlides(currentSlide, nextSlide);

					nextSlide.animate({}, 1, function () {
						jQuerythis.children(".unoslider_blocks_area").remove();
						jQuerythis.children(".unoslider_cube").remove();

						var layerDelay = jQuerythis.layer.show(nextSlide);

						if((handChange === true && (cfg.slideshow.continuous === true || !cfg.slideshow)) || handChange === false){
							running = false;
							layerTimer = setTimeout(function(){
								layerRunning = false;
								if(stoped === false) jQuerythis.slideshow.restart();
							}, layerDelay);
						}

						if(cfg.tooltip) jQuerythis.tooltip.show(nextSlide);
					});
				});
			},
			...
			presets: function(){
				...
				if(typeof cfg === 'object' && typeof cfg.preset === 'object'){
					...
					if(typeof cfg.preset[index] == 'string'){
						if(cfg.preset[index] in jQuerythis.preset){
							jQuerythis.preset[cfg.preset[index]]();
						}else{
							alert('There is no preset named "'+cfg.preset[index]+'"');
						}
					}else{
						cfg.animation = jQuery.extend({}, preset.animation, cfg.preset[index].animation);
						cfg.block = jQuery.extend({}, preset.block, cfg.preset[index].block);
					}
					...
					if(cfg.preset) jQuerythis.preset.init();
				}
			},
			...
			mobilePresets: function(){
				...
				jQuerythis.touch.reverse = false;
			},

			// responsive design enabled
			responsive: function(){
				var base_width = jQuerythis.width();
				var base_height = jQuerythis.height();
				var ratio = (base_width / base_height);
				jQuerythis.data('base_width', base_width);
				jQuerythis.data('base_height', base_height);

				// fit to width
				function fit(){
					if(jQuerythis.parent().width() < (base_width)){
						jQuerythis.width('100%');
						jQuerythis.find('.unoslider_slider_area').width('100%');
					}else{
						jQuerythis.css({width: base_width, height: base_height });
						jQuerythis.find('.unoslider_slider_area').css({width: base_width, height: base_height });
					}

					var cropHeight = Math.floor(((jQuerythis.width()/ratio)/1))*1;
					jQuerythis.height(cropHeight);
					jQuerythis.find('.unoslider_slider_area').height(cropHeight);
				}
			...
			setTransition: function(currentSlide, nextSlide, handChange){
				...
				if(cfg.mobile && jQuerythis.main.is_mobile()){
					this.mobilePresets();
				}else{
					this.presets();
				}
				...
				jQuerythis.transition[final_transition](currentSlide, nextSlide, handChange);
			...
			setDirection: function(){
				...
				return jQuerythis.direction[final_direction]();
			}
		...
		jQuerythis.touch = {
			...
			init: function(){
				...
				if ('ontouchstart' in document.documentElement) {
					...
					jQuerythis.find('.unoslider_left').remove();
					jQuerythis.find('.unoslider_right').remove();

					jQuerythis[0].addEventListener('touchstart', jQuerythis.touch.start, false);
				}
			},
			...
			start: function(event){
				if (event.touches.length == 1) {
					...
					this.moving = true;
					jQuerythis[0].addEventListener('touchmove', jQuerythis.touch.move, false);
				}
			},
			...
			move: function(event){
				if(this.moving) {
					...
					if(Math.abs(dx) >= 50) {
						jQuerythis.touch.end();
						if(dx > 0) {
							jQuerythis.touch.left();
						} else {
							jQuerythis.touch.right();
						}
					}
				}
			},
			...
			// touch end
				...
				jQuerythis[0].removeEventListener('touchmove', jQuerythis.touch.move);
				this.startX = null;
				this.moving = false;
			},
			...
			right: function(){
				if(running === false){
					jQuerythis.touch.reverse = true;
					jQuerythis.slideshow.prev();
				}
			}
		...
		jQuerythis.layer = {
			...
			// generate layers
			generate: function(){
				jQuerythis.find('.unoslider_layers').children().hide();
			},

			responsive: function(){
				jQuerythis.find('.unoslider_layers').children().each(function(){
					...
					var responsive_width = (original_width / jQuerythis.data('base_width')) * 100;
					var responsive_height = (original_height / jQuerythis.data('base_height')) * 100;
					var responsive_left = (original_left / jQuerythis.data('base_width')) * 100;
					var responsive_top = (original_top / jQuerythis.data('base_height')) * 100;
			...
			scale_text: function(){
				...
				var resizer = function(init) {
					jQuerythis.find('.unoslider_layers').children().each(function(){
						...
						var scale = (jQuerythis.width() * 100) / jQuerythis.data('base_width');
      	...
      	jQuery(window).resize(function(){
			...
			// start animation
			show: function(slide){
				jQuerythis.find('.unoslider_layers').children().css({'z-index': 1, position: 'absolute'}).hide();

				var layers = jQuery(slide).find('.unoslider_layers').children().length;
				var delays = [];
				var delay = 250;
				var duration = 300;
				var i;
				...
				i = 1;
				jQuery(slide).find('.unoslider_layers').children().each(function(){
					delays.push(delay * i);
					i++;
				});
				...
				jQuery(slide).find('.unoslider_layers').children().each(function(){
					...
					for(x in tridy){
						if(jQueryself.hasClass(tridy[x])){
							animation =  jQuerythis.layer[tridy[x]]();
							break;
						}
					}

					jQueryself.css(animation['css']);
					layerAnimation[i] = jQueryself.delay(delays[i]).animate(animation['animate'], {duration: duration});
			...
			slide_top: function(){
				var slider_height = jQuerythis.data('base_height');
			...
			slide_right: function(){
				var slider_width = jQuerythis.data('base_width');
		...
		jQuerythis.timebar = {
			...
			pause: function(){
				...
				var timebar = jQuerythis.find('.unoslider_timer');
			...
			resume: function(){
				var timer = jQuerythis.find('.unoslider_timer');
				...
				var baseTimeout = (jQuerythis.timer.speed() * 1000);
			...
			// create timebar
			set: function(){
				return jQuery('<div class="unoslider_timer"></div>').css({'z-index': 1}).width('0%').appendTo(jQuerythis[0]);
			},

			start: function(delay){
				var remaining = delay || jQuerythis.timer.speed() * 1000;
				var timer = jQuerythis.find('.unoslider_timer');
				...
				timer.animate({
					...
					jQuerythis.timebar.hide();
				});
			},

			stop: function(){
				var timebar = jQuerythis.find('.unoslider_timer');
				jQuery(timebar).stop().width('0%').hide();
			},

			hide: function(){
				jQuerythis.find('.unoslider_timer').width('0%').hide();
			}
		...
		jQuerythis.timer = {
			...
			start: function(delay){
				...
				interval[uid] = setTimeout(function(){
					...
					jQuerythis.slideshow.changeTo(activeSlideNum, false);
		...
		jQuerythis.navigation = {
			...
			generate: function(){
				...
				if(cfg.navigation){
					var container = jQuery('<div class="unoslider_navigation_container"></div>').appendTo(jQuerythis[0]);

					if(cfg.slideshow){
						var play = jQuery('<a title="'+cfg.navigation.play+'" class="unoslider_play unoslider_navigation">'+cfg.navigation.play+'</a>').appendTo(container);
						var stop = jQuery('<a title="'+cfg.navigation.stop+'" class="unoslider_pause unoslider_navigation">'+cfg.navigation.stop+'</a>').appendTo(container);
					...
					jQuery('<a title="'+cfg.navigation.prev+'" class="unoslider_left unoslider_navigation">'+cfg.navigation.prev+'</a>').appendTo(container);
					jQuery('<a title="'+cfg.navigation.next+'" class="unoslider_right unoslider_navigation">'+cfg.navigation.next+'</a>').appendTo(container);

					jQuerythis.find('.unoslider_right').click(function(){
						if(running === false){
							jQuerythis.slideshow.next();
						}
					});

					jQuerythis.find('.unoslider_left').click(function(){
						if(running === false){
							jQuerythis.slideshow.prev();
						}
					});

					jQuerythis.find('.unoslider_pause').click(function(){
						if(!stoped){
							stoped = true;
							self.changeState('stop');
							jQuerythis.slideshow.stop();
						}
					});

					jQuerythis.find('.unoslider_play').click(function(){
						if(stoped){
					...
					jQuery(".unoslider_navigation").css({
						'z-index': 5,
						position: 'absolute'
					});
				}
			...
			autohide: function(){
				...
				var container = jQuerythis.find(selector);
			...
			changeState: function(state){
				if(state == 'stop'){
					jQuerythis.find('.unoslider_play').removeClass('unoslider_hide').show();
					jQuerythis.find('.unoslider_pause').addClass("unoslider_hide").hide();
				}else if(state == 'play'){
					jQuerythis.find('.unoslider_play').addClass("unoslider_hide").hide();
					jQuerythis.find('.unoslider_pause').removeClass('unoslider_hide').show();
				}
			},

			// create bullet indicator
			indicator: function(){
				var appendTo = cfg.indicator.position || jQuerythis[0];
				var activeClass = 'unoslider_indicator_active';

				controls = jQuery('<div class="unoslider_indicator"></div>').css({'z-index': 6});

				!cfg.indicator.position ? controls.appendTo(appendTo) : controls.insertAfter(appendTo);

				slides.each(function(slide) {
					jQuery('<a title="' + (slide + 1) + '" class="'+activeClass+'">' + (slide + 1) + '</a>').appendTo(controls).bind("click", function(){
						if(running === false) jQuerythis.slideshow.changeTo(slide, true);
					});
		...
		jQuerythis.slideshow = {

			// stop slideshow on mouseover
			pauseOnHover: function(){
				jQuerythis.hover(function(){
					if(running === false && !stoped && layerRunning === false){
						jQuerythis.timebar.pause();
						jQuerythis.timer.pause();
					}
				}, function(){
					if(running === false && !stoped && layerRunning === false){
						jQuerythis.timebar.resume();
						jQuerythis.timer.resume();
					}
				});
			},

			start: function(){
				jQuery('.unoslider_preloader').remove();
				...
				if(cfg.slideshow){
					if(!cfg.slideshow.infinite && !force && activeSlideNum == (slides.length - 1)){ // stop slideshow on last slide
						jQuerythis.slideshow.stop();
						jQuerythis.navigation.changeState('stop');
					}else{ // continue slideshow
						jQuerythis.timebar.start();
						jQuerythis.timer.start(null);
						force = false;
					}
				}
			},
			...
			changeTo: function(changeTo, handChange){
				...
				jQuery.each(layerAnimation, function(i){
				...
				jQuerythis.main.setTransition(currentSlide, nextSlide, handChange);
		...
		jQuerythis.tooltip = {
			...
			generate: function(){
				if(cfg.tooltip){
					...
					jQuerythis.find('.unoslider_caption').removeClass('unoslider_caption').addClass('unoslider_caption_data').hide();
					...
					slides.each(function(){
						...
						if(img.length >= 1){
							...
							if(typeof caption !== 'undefined'){
								...
								jQuery('<div class="unoslider_caption_data">'+caption+'</div>').appendTo(this).hide();
							}
						}
					});

					jQuery('<div class="unoslider_caption"></div>').hide().css({'z-index': 5}).appendTo(jQuerythis);
				}else{
					jQuerythis.find('.unoslider_caption').remove();
				}

				this.height = jQuerythis.find('.unoslider_caption').height();
			},

			show: function(slide){
				var html = jQuery(slide).find('.unoslider_caption_data').html();
				if(html) jQuerythis.find('.unoslider_caption').html(html).fadeIn();
			},

			hide: function(){
				jQuerythis.find('.unoslider_caption').fadeOut();
			}
		};
		...
		jQuerythis.preset = {
			...
			init: function(){
				if(cfg.preset in jQuerythis.preset){
					jQuerythis.preset[cfg.preset]();
				}else{
			...
			autosize: function(){
				var ratio = jQuerythis.width() / jQuerythis.height();
		...
		jQuerythis.transition = {
			...
			animation: function(direction, animate, duration){
				...
				while (row <= rows) {
					while (col <= columns) {
						...
						jQuerythis.find("#block_" + item).delay(direction['zpozdeni'][item]).animate(animate, {duration: duration});
						col++;
					}
					row++; col = 1;
				}
			...
			base_animation: function(css, animate, delayed, direction, currentSlide, nextSlide, handChange){
				...
				jQuerythis.find('.unoslider_cube').css(css);

				this.animation(direction, animate, duration);

				jQuerythis.main.execute(direction, duration, currentSlide, nextSlide, handChange, delayed);
			},
			move_animation: function(css, animate, direction, opacity, currentSlide, nextSlide, handChange){
				var duration = this.speed();

				jQuerythis.find('.unoslider_cube').css({
					display: 'block',
					opacity: opacity
				});

				jQuerythis.find('.unoslider_blocks').css(css);

				this.animation(direction, animate, duration);

				jQuerythis.main.execute(direction, duration, currentSlide, nextSlide, handChange, true);
			},
			drop_animation: function(animate, direction, currentSlide, nextSlide, handChange){
				...
				jQuerythis.main.execute(direction, duration, currentSlide, nextSlide, handChange, false);
			},
			...
			alternate_animation: function(css_even, css_odd, animate_even, animate_odd, direction, currentSlide, nextSlide, handChange, delayed){
				...
				jQuerythis.find('.unoslider_cube:even').css(css_even);

				jQuerythis.find('.unoslider_cube:odd').css(css_odd);

				while (row <= rows) {
					while (col <= columns) {
						item = (columns * row - (columns - col));

						jQuerythis.find("#block_" + item).delay(direction['zpozdeni'][item]).animate(item % 2 == 1 ? animate_even : animate_odd, { duration: duration });
			...
			fade: function(currentSlide, nextSlide, handChange){
				var direction = jQuerythis.main.prepare(currentSlide);
			...
			flash: function(currentSlide, nextSlide, handChange){
				...
				while (row <= rows) {
					while (col <= columns) {
						...
						jQuerythis.find(selector).delay(direction['zpozdeni'][item]).animate({
							opacity: 0
						}, {
							duration: duration
						});

						jQuerythis.find(selector).children().delay(direction['zpozdeni'][item]).animate({
							opacity: 0
						}, {
							duration: 0
						});
			...
			appear_topleft: function(currentSlide, nextSlide, handChange){
				var direction = jQuerythis.main.prepare(nextSlide);
		...
		jQuerythis.direction = {
			...
			horizontal_bottom: function(){
				var top = jQuerythis.direction.horizontal_top();
			...
			horizontal_bottomleft: function(){
				var topRight = jQuerythis.direction.horizontal_topright();
			...
			horizontal_bottomright: function(){
				var topLeft = jQuerythis.direction.horizontal_topleft();
			...
			vertical_right: function(){
				var left = jQuerythis.direction.vertical_left();
			...
			vertical_topright: function(){
				var bottomLeft = jQuerythis.direction.vertical_bottomleft();
			...
			vertical_bottomright: function(){
				var topLeft = jQuerythis.direction.vertical_topleft();
			...
			diagonal_bottomright: function(){
				var topLeft = jQuerythis.direction.diagonal_topleft();
			...
			diagonal_bottomleft: function(){
				var topRight = jQuerythis.direction.diagonal_topright();
			...
			spiral_bottomleft: function(){
				var topRight = jQuerythis.direction.spiral_topright();
			...
			spiral_bottomright: function(){
				var topLeft = jQuerythis.direction.spiral_topleft();
			...
			horizontal_zigzag_bottomright: function(){
				var topLeft = jQuerythis.direction.horizontal_zigzag_topleft();
			...
			horizontal_zigzag_bottomleft: function(){
				var topRight = jQuerythis.direction.horizontal_zigzag_topright();
			...
			vertical_zigzag_bottomright: function(){
				var topLeft = jQuerythis.direction.vertical_zigzag_topleft();
			...
			vertical_zigzag_bottomleft: function(){
				var topRight = jQuerythis.direction.vertical_zigzag_topright();
		...
		jQuerythis.preloader = {

			// initialize preloader
			init: function(callback){
				var images = jQuerythis.find('img');
				...
				slides.children().each(function(){
					...
					if(bg !== 'none'){
						...
						bgImage.src = bg.replace(/"/g,"").replace(/url\(|\)jQuery/ig, "");
						images.push(bgImage);
					}
				});

				if(images.length > 0){
					jQuery('<div class="unoslider_preloader"></div>').css({
						position: 'absolute',
						textAlign: 'center',
						width: '100%'
					...
					images.each(function(){
						var img = new Image;
						var jQuerythisImg = jQuery(this);

						img.src = this.src;

						if (!img.complete) {
							jQuery(img).bind("load error", function(){
								jQuerythis.preloader.show(jQuerythisImg, callback, imagesCount);
							});
						} else {
							jQuerythis.preloader.show(jQuerythisImg, callback, imagesCount);
						}
					...
					jQuery(lists[0]).css("display", "block").animate({
						opacity: 1
					}, 500);
			...
			show: function(img, callback, imagesCount){
				...
				var loader = jQuerythis.find('.unoslider_preloader');
				...
				switch(cfg.preloader){
					case 'spinner':
						jQuery(loader).empty();
						jQuery('<div class="unoslider_spinner"></div>').css({width: '100%', height: '100%'}).appendTo(loader);
						break;
					default:
						jQuery(loader).empty();
						var progress = jQuery('<span></span>').css({width: (loaded / imagesCount * 100).toFixed(0)+'%'});
						jQuery(loader).css({'padding-top': jQuerythis.height() / 2 +'px', height: '50%'});
						jQuery('<div class="unoslider_progress"></div>').appendTo(loader).append(progress);
						break;
				}
			}
		};
		...
		this.play = function(){
			if(stoped){
				jQuerythis.navigation.changeState('play');
				jQuerythis.slideshow.start();
				stoped = false;
			}
		};
		this.next = function(){
			if(running === false) jQuerythis.slideshow.next();
		};
		this.prev = function(){
			if(running === false) jQuerythis.slideshow.prev();
		};
		this.goto = function(num){
			if(running === false){
				if(num > slides.size()){
					...
					jQuerythis.slideshow.changeTo(num-1, true);
				}
			}
		};
		this.cfg = function(config){
			jQuery.extend(true, cfg, config);
		};

	};

	jQuery.fn.unoslider = function(options){
		return (new jQuery.UnoSlider(options, jQuery(this)));
	};
