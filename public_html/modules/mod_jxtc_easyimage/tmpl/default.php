...
// Setup some handy vars used to build the output
...
$tooltip = $tooltipType; if ($tooltip == 'text2') { $tooltip = 'text'; }
...
// Load standard js/css files on <head>
...
foreach ($images as $image) {
  ...
  if ($activeLinks && $image->url) { echo '</a>'; }
	if ($params->get('li_caption',0)) { echo '<div>'.$captionHTML.'</div>'; }
	echo '</li>';
}
echo '</ul></div>';