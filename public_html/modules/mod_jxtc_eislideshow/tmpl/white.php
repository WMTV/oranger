...
.$jxtc-images {	display: block; overflow: hidden; position: relative;	margin-bottom:".($thumbnails*($theight+12))."px;}		
...
.$jxtc-images img {	float: left; left: 0; top: 0; background:#fff}	
...
.$jxtc-thumbnails {	bottom: -".($theight+12)."px; height: ".($theight+12)."px; left: 0; position: absolute; width: 100%;	background:#fff;}
...
.$jxtc-thumbnails-hidden {	background-color: #fff; opacity: 0;}
.$jxtc-thumbnails-inactive {	background-color: #fff; opacity: .5;}
.$jxtc-thumbnails-active {	background-color: #222; opacity: 1;}
";
